...
//     apiKey: 'AIzaSyDEMZlZQwEHRCSVPjkcUBmbhEnFRJnFS80',
// });
...
//     const submitQuiz = () => {
//         clearInterval(timerRef.current); // Stop live timer
...
//                     Difficulty: {difficulty} | Total no of questions: {questionCount}
//                 </p>
...
export default function QuizPage() {
    ...
    useEffect(() => {
        ...
        if ((!locationState.topic || !locationState.questionCount || !topic || !questionCount) && !saved) {
            navigate('/home');
        }
...
[
  {
    ...
    // ✅ Shuffle answers when currentIndex changes
    useEffect(() => {
        if (questions.length > 0) {
            const current = questions[currentIndex];
            const options = [...current.options];
    ...
    }, [answers, currentIndex, elapsedTime, questions, topic, difficulty, questionCount]);
    ...
    const submitQuiz = () => {
        ...
        const prev = JSON.parse(localStorage.getItem('quizResults')) || [];
        localStorage.setItem('quizResults', JSON.stringify([...prev, result]));
        localStorage.removeItem('resumeQuiz'); // ✅ Clear resume data
    };
